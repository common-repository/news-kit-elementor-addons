...
class Grid_Module extends \Nekit_Widget_Base\Base {
    ...
    protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors'	=>	[
					'{{WRAPPER}} .nekit-grid-widget .post-thumb-wrap' => 'padding-bottom: calc( {{SIZE}} * 100% );'
				]
			]
		);
		...
		$this->add_responsive_control(
            ...
            [
                ...
                'label_block'   =>  true,
                'selectors' =>  [
                    '{{WRAPPER}} .post-thumb-wrap, {{WRAPPER}} .post-thumb-wrap'  =>  'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}}'
                ]
            ]
        );
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .nekit-grid-widget .news-grid-post-wrap' => 'column-gap: {{SIZE}}{{UNIT}};'
				]
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .nekit-grid-widget .news-grid-post-wrap' => 'row-gap: {{SIZE}}{{UNIT}};'
				]
			]
		);
        $this->end_controls_section();
		...
		if( $this::$widget_count != 'three' ) :
			$this->start_controls_section(
				'post_numbering_section_typography',
				[
					'label'	=>	esc_html__( 'Post Numbering Typography', 'news-kit-elementor-addons' ),
					'tab'	=>	\Elementor\Controls_Manager::TAB_STYLE
				]
			);
				...
				$this->add_group_control(
					...
					[
						...
						'name'	=>	'post_numbering_typography',
						'selector'	=>	'{{WRAPPER}} .post-count'
					]
				);

				$this->add_control(
					'post_numbering_color',
					[
										...
										<div class="nekit-item-box-wrap">
											<?php if( $settings['show_post_thumbnail'] = 'yes' ) : ?>
												<figure class="post-thumb-wrap">
													...
													<?php if( $settings['show_post_categories'] == 'yes' && $this::$widget_count != 'two' ) nekit_get_post_categories( get_the_ID(), 2 ); ?>
												</figure>
											<?php endif; ?>
