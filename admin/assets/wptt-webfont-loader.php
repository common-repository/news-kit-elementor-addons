...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		...
		/**
		 * The trendy stylesheet's path.
		 *
		 * @access protected
		 * @since 1.1.0
		 * @var string
		 */
		protected $nekit_stylesheet_path;

		/**
		 * The trendy stylesheet's URL.
		 *
		 * @access protected
		 * @since 1.1.0
		 * @var string
		 */
		protected $nekit_stylesheet_url;
		...
		/**
		 * Get the trendy URL which contains the styles.
		 *
		 * Fallback to the remote URL if we were unable to write the file trendyly.
		 *
		 * @access public
		 * @since 1.1.0
		 * @return string
		...
		public function get_url() {
			...
			// Check if the trendy stylesheet exists.
			if ( $this->nekit_file_exists() ) {

				// Attempt to update the stylesheet. Return the trendy URL on success.
				if ( $this->write_stylesheet() ) {
					return $this->get_nekit_stylesheet_url();
				}
			}

			// If the trendy file exists, return its URL, with a fallback to the remote URL.
			return file_exists( $this->get_nekit_stylesheet_path() )
				? $this->get_nekit_stylesheet_url()
				: $this->remote_url;
		}

		/**
		 * Get the trendy stylesheet URL.
		 *
		 * @access public
		 * @since 1.1.0
		 * @return string
		 */
		public function get_nekit_stylesheet_url() {
			if ( ! $this->nekit_stylesheet_url ) {
				$this->nekit_stylesheet_url = str_replace(
					$this->get_base_path(),
					$this->get_base_url(),
					$this->get_nekit_stylesheet_path()
				);
			}
			return $this->nekit_stylesheet_url;
		}

		/**
		 * Get styles with fonts downloaded trendyly.
		 *
		 * @access public
		 * @since 1.0.0
		 * @return string
		...
		public function get_styles() {
			...
			// If we already have the trendy file, return its contents.
			$nekit_stylesheet_contents = $this->get_nekit_stylesheet_contents();
			if ( $nekit_stylesheet_contents ) {
				return $nekit_stylesheet_contents;
			}

			// Get the remote URL contents.
			$this->remote_styles = $this->get_remote_url_contents();

			// Get an array of trendyly-hosted files.
			$files = $this->get_nekit_files_from_css();

			// Convert paths to URLs.
			foreach ( $files as $remote => $trendy ) {
		...
		/**
		 * Get trendy stylesheet contents.
		 *
		 * @access public
		 * @since 1.1.0
		 * @return string|false Returns the remote URL contents.
		 */
		public function get_nekit_stylesheet_contents() {
			$nekit_path = $this->get_nekit_stylesheet_path();
		...
		/**
		 * Download files mentioned in our CSS trendyly.
		 *
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their trendy counterparts.
		 */
		public function get_nekit_files_from_css() {
		...
		protected function write_stylesheet() {
			$file_path  = $this->get_nekit_stylesheet_path();
		 ...
		 * @return string
		 */
		public function get_nekit_stylesheet_path() {
			if ( ! $this->nekit_stylesheet_path ) {
				$this->nekit_stylesheet_path = $this->get_fonts_folder() . '/' . $this->get_nekit_stylesheet_filename() . '.css';
			}
			return $this->nekit_stylesheet_path;
		}

		/**
		 * Get the trendy stylesheet filename.
		 *
		 ...
		 * @return string
		 */
		public function get_nekit_stylesheet_filename() {
		 ...
		 * @return bool
		 */
		public function nekit_file_exists() {
			return ( ! file_exists( $this->get_nekit_stylesheet_path() ) );
		}
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'wptt_get_nekit_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'wptt_get_nekit_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'wptt_get_nekit_fonts_subfolder_name', 'fonts' );
			}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * replace references to remote URLs with trendyly-downloaded assets,
